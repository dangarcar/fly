...
void RouteDialog::render(const Renderer& renderer) {
    ...
    for(int i=0; i<int(route.planes.size()); ++i) {
        int x = dialog.x + dialog.w/2 - planesW/2 + i*100;
        auto fillPercentage = float(route.planes[i].pass.size()) / air::PLANE_CAPACITY_PER_LEVEL[route.level];
        auto color = air::FULL_GRADIENT.getColor(fillPercentage);
        auto tex = air::PLANE_TEXTURE_PER_LEVEL[route.level];
        renderer.renderExt(tex, x, dialog.y + 120, 90.0f, 0, true, color);
    }
